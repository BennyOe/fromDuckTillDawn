...
class SpectorCancelInvestigating(
    @JvmField
    ...
    override fun copyTo(task: Task<SpectorContext>): Task<SpectorContext> {
        (task as SpectorCancelInvestigating).duration = duration
        return task
    }

    companion object {
        val logger = logger<SpectorCancelInvestigating>()
    }
}
